...
function itemCard(s: IItem): HTMLDivElement {
  const item = cloudItem.find((itm) => itm.id === s.itemId)!
...
export class Bag implements IPMC {
      ...
      <div class="con">
        ...
        <div class="board">
          <div class="economies">
            ...
            <div class="eco eco-hexsa">
              <img src="/assets/items/cloud/hexsa.png" alt="perisma"/>
              <i>0</i>
            </div>
  ...
  private updateEconomies(): void {
    ...
    const ehexsa = futor(".economies .eco-hexsa i", this.el)
  ...
  private writeItems(ktype: string): void {
    ...
    const pageItems = Object.keys(this.items!).filter((k) => cloudItem.find((citm) => citm.id === this.items![k].itemId)!.group === ktype && (!this.items![k].expiry || this.items![k].expiry > Date.now()) && this.items![k].amount >= 1)
